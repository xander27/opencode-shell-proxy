...
- The plugin patches the in-process `fetch` function with an explicit HTTP
  proxy option, so TUI LLM requests are proxied even when opencode creates a
  direct provider connection before directory plugins load.
- The plugin waits for the initial bridge startup and health check before
  returning from initialization. The first LLM request therefore waits for
  gost download, bridge startup, and verification instead of going direct.
...
- On a cold start, opencode startup may take several seconds while `gost` is
  downloaded and the proxy chain is checked. If the check fails, traffic goes
  direct and a warning is shown.
